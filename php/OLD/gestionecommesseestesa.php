...
function distintaRisorse(PDO $conn_mes, string $idProduzione, string | null $idLineaProduzione, string | null $idProdotto)
{
		...
		if (!$drcs) {
			...
			foreach ($risorse as $risorsa) {
				$risorseCoinvolte[] = [
					...
					'rc_DataOraInizio' => null,
					'rc_DataOraFine' => null,
					'rc_VelocitaRisorsa' => null,
			...
			foreach ($drcs as $drc) {
				$risorseCoinvolte[] = [
					...
					'rc_DataOraInizio' => null,
					'rc_DataOraFine' => null,
					'rc_VelocitaRisorsa' => $drc['drc_Velocita'],
	...
	foreach ($risorseCoinvolte as $risorsaCoinvolta) {
		...
		$sth = $conn_mes->prepare(
			...
			LEFT JOIN velocita_teoriche ON vel_IdProdotto = op_Prodotto AND op_LineaProduzione = vel_IdLineaProduzione
			WHERE op_IdProduzione = :IdProduzione"
		);
		$sth->execute(['IdProduzione' => $risorsaCoinvolta['rc_IdProduzione']]);
		$infoOrdine = $sth->fetch();
		if (isset($risorsaCoinvolta['rc_DataOraInizio'])) {
			$dataOraInizio = new DateTime($risorsaCoinvolta['rc_DataOraInizio']);
			$risorsaCoinvolta['rc_DataOraInizio'] = $dataOraInizio->format('d/m/Y - H:i');
		} else {
			$dataProduzione = new DateTime($infoOrdine['op_DataProduzione'] . ' ' . $infoOrdine['op_OraProduzione']);
			$interval = 5 * ($risorsaCoinvolta['ris_Ordinamento'] - 1);
			if ($interval != 0) {
				$dataProduzione->add(new DateInterval('PT' . $interval . 'M'));
			}
			$dataOraInizio = $dataProduzione;
			$risorsaCoinvolta['rc_DataOraInizio'] = $dataProduzione->format('d/m/Y - H:i');
		}

		if (isset($risorsaCoinvolta['rc_DataOraFine'])) {
			$dataOraFine = new DateTime($risorsaCoinvolta['rc_DataOraFine']);
			$risorsaCoinvolta['rc_DataOraFine'] = $dataOraFine->format('d/m/Y - H:i');
		} else {
			$qta = $infoOrdine['op_QtaDaProdurre'];

			if (isset($risorsaCoinvolta['rc_VelocitaRisorsa'])) {
				$velocita = $risorsaCoinvolta['rc_VelocitaRisorsa'];
			} else {
				$velocita = $infoOrdine['vel_VelocitaTeoricaLinea'];
			}

			if ($velocita != 0){
				$tempoSec = $qta / ($velocita * 3600);

				$dataOraFine = new DateTime();
				$dataOraFine->setTimestamp($dataOraInizio->getTimestamp() + $tempoSec);
				$risorsaCoinvolta['rc_DataOraFine'] = $dataOraFine->format('d/m/Y - H:i');
			} else {
				$risorsaCoinvolta['rc_DataOraFine'] = $risorsaCoinvolta['rc_DataOraInizio'];
			}
		...
		$output[] = [
			...
			'rc_DataOraInizio' => $risorsaCoinvolta['rc_DataOraInizio'],
			'rc_DataOraFine' => $risorsaCoinvolta['rc_DataOraFine'],
			'rc_VelocitaRisorsa' => $risorsaCoinvolta['rc_VelocitaRisorsa'],
...
function distintaComponenti(PDO $conn_mes, string $idProduzione, string | null $idProdotto, float | null $qtaDaProdurre)
{
	...
	foreach ($componenti as $componente) {
		$output[] = [
			...
			'Fabbisogno' => (!empty($qtaDaProdurre) ? (ceil(($qtaDaProdurre * $componente['cmp_FattoreMoltiplicativo']) / $componente['cmp_PezziConfezione'])) . ' ' . $componente['um_Sigla'] : ''),
...
if (!empty($_REQUEST['azione'])) {
	...
	if ($_REQUEST['azione'] == 'mostra') {
		...
		$sth = $conn_mes->prepare(
			...
			WHERE OP.op_Stato IN " . $stringArray
		);
		$sth->execute($filtri);
		...
		foreach ($ordini as $ordine) {
			...
			$output[] = [
				'op_IdProduzione' => ($ordine['op_Riferimento'] != '' ? $ordine['op_IdProduzione'] . " (" . $ordine['op_Riferimento'] . ")" : $ordine['op_IdProduzione']),
	...
	if ($_REQUEST['azione'] == 'salva-ordine') {
		...
		try {
			...
			$sth = $conn_mes->prepare(
				"INSERT INTO risorse_coinvolte(rc_IdProduzione, rc_IdRisorsa, rc_LineaProduzione, rc_NoteIniziali,  rc_RegistraMisure, rc_FlagUltima, rc_IdRicetta, rc_DataOraInizio, rc_DataOraFine, rc_VelocitaRisorsa)
				VALUES(:rc_IdProduzione, :rc_IdRisorsa, :rc_LineaProduzione, :rc_NoteIniziali,  :rc_RegistraMisure, :rc_FlagUltima, :rc_IdRicetta, :rc_DataOraInizio, :rc_DataOraFine, :rc_VelocitaRisorsa)"
			);
			foreach ($risorseCoinvolte as $risorsaCoinvolta) {
...
if ($allineamento) {
	require_once("../inc/importaDatiERP.php");
}
?>
<!DOCTYPE html>
<html lang="it">
...
<body>
									...
									<div class="row">
										...
										<div class="col-lg-2">
											<div class="form-group">
												...
												<input type="number" class="form-control form-control-sm dati-popup-modifica obbligatorio" id="op_QtaDaProdurre" name="op_QtaDaProdurre" aria-label="" aria-describedby="inputGroup-sizing-lg" required>
											</div>
										</div>
										...
										<div class="col-lg-2">
											<div class="form-group">
												...
												<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica obbligatorio" id="op_DataProduzione" name="op_DataProduzione" aria-label="" aria-describedby="inputGroup-sizing-lg" value="" required>
											</div>
										</div>
										...
										<div class="col-lg-2">
											<div class="form-group">
												...
												<select class="form-control form-control-sm selectpicker test-disabilitato" id="op_Stato" name="op_Stato" required>
													<?php
													$sth = $conn_mes->prepare(
										...
										<div class="col-lg-3">
											<div class="form-group">
												<label class="label-evidenziata" for="op_LineaProduzione">Linee disponibili</label><span style='color:red'> *</span>
												<select class="form-control form-control-sm selectpicker dati-popup-modifica" id="op_LineaProduzione" name="op_LineaProduzione" required>
													<?php
													$sth = $conn_mes->prepare(
										...
										<div class="col-lg-2">
											<div class="form-group">
												...
												<input type="number" class="form-control form-control-sm dati-ordine obbligatorio" id="vel_VelocitaTeoricaLinea" name="vel_VelocitaTeoricaLinea" aria-label="" aria-describedby="inputGroup-sizing-lg" required>
											</div>
										</div>
										...
										<div class="col-lg-1">
											<div class="form-group">
												...
												<input type="number" class="form-control form-control-sm dati-popup-modifica obbligatorio" id="op_Priorita" name="op_Priorita" aria-label="" aria-describedby="inputGroup-sizing-lg" required>
											</div>
										</div>
										<div class="col-lg-2">
											<div class="form-group">
												<label class="label-evidenziata" for="op_Lotto">Lotto</label><span style='color:red'> *</span>
												<input type="text" class="form-control form-control-sm dati-ordine obbligatorio" id="op_Lotto" name="op_Lotto" aria-label="" aria-describedby="inputGroup-sizing-lg" required>
											</div>
										</div>
										...
										<div class="col-lg-2">
											<div class="form-group">
												...
												<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica obbligatorio" id="op_DataFine" name="op_DataFine" aria-label="" aria-describedby="inputGroup-sizing-lg" value="" required>
											</div>
										</div>
								...
								<div class="tab-content">
									...
									<div aria-labelledby="tab-risorse" class="tab-pane fade show active" id="risorse" role="tabpanel">
										...
										<div class="table-responsive">
											...
											<table id="tabellaDistintaRisorse" class="table table-striped" style="width:100%" data-source="gestionecommesseestesa.php?azione=mostra-dettagli-distinte">
												<thead>
													<tr>
														<th>Id macchina</th>
														<th>Descrizione</th>
									...
									<div aria-labelledby="tab-componenti" class="tab-pane fade" id="componenti" role="tabpanel">
										...
										<div class="table-responsive">
											...
											<table id="tabellaDistintaComponenti" class="table table-striped" style="width:100%" data-source="gestionecommesseestesa.php?azione=mostra-dettagli-componenti">
												<thead>
													<tr>
														<th>Id componente</th>
														<th>Descrizione</th>
									...
									<div aria-labelledby="tab-consumi" class="tab-pane fade" id="consumi" role="tabpanel">
										...
										<div class="table-responsive">
											...
											<table id="tabellaDistintaConsumi" class="table table-striped" style="width:100%" data-source="gestionecommesseestesa.php?azione=mostra-dettagli-componenti">
												<thead>
													<tr>
														<th>Macchina</th>
														<th>Tipo consumo</th>
						...
						<div class="row">
							...
							<div class="col-12">
								<div class="form-group">
									...
									<input type="text" class="form-control form-control-sm dati-popup-modifica" name="op_Lotto" id="op_Lotto" autocomplete="off">
								</div>
							</div>
							<div class="col-6">
								<div class="form-group">
							...
							<div class="col-6">
								<div class="form-group">
									...
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica obbligatorio" name="DataOraOrdine" id="DataOraOrdine" required>
								</div>
							</div>
							<div class="col-6">
								<div class="form-group">
									<label for="DataOraProduzione">Data pianificazione</label><span style='color:red'> *</span>
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica obbligatorio" name="DataOraProduzione" id="DataOraProduzione" required>
								</div>
							</div>
							<div class="col-12">
								<div class="form-group">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="forms-sample" id="form-nuova-risorsa">
						...
						<div class="row">
							...
							<div class="col-12">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" id="rc_IdRisorsa" name="rc_IdRisorsa" required>
							...
							<div class="col-12">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" id="rc_IdRicetta" name="rc_IdRicetta" required>
							...
							<div class="col-4">
								<div class="form-group">
									<label for="rc_DataOraInizio">Inizio previsto</label><span style='color:red'> *</span>
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica" name="rc_DataOraInizio" id="rc_DataOraInizio">

								</div>
							</div>
							<div class="col-4">
								<div class="form-group">
									<label for="rc_VelocitaRisorsa">Velocità</label><span style='color:red'> *</span>
									<input type="number" class="form-control form-control-sm dati-popup-modifica" name="rc_VelocitaRisorsa" id="rc_VelocitaRisorsa">

								</div>
							</div>
							<div class="col-4">
								<div class="form-group">
									<label for="rc_DataOraFine">Fine prevista</label><span style='color:red'> *</span>
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica" name="rc_DataOraFine" id="rc_DataOraFine">
						...
						<input type="hidden" id="ris_Ordinamento" name="ris_Ordinamento" value="">
						...
						<div class="row">
							<div class="col-12">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control form-control-sm dati-popup-modifica" name="cmp_IdProduzione" id="cmp_IdProduzione" readonly>
								</div>
							</div>
							<div class="col-12">
								<div class="form-group">
									<label for="cmp_Componente">Componente</label><span style='color:red'> *</span>
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" id="cmp_Componente" name="cmp_Componente" data-live-search="true" required>
							...
							<div class="col-4">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" name="cmp_Udm" id="cmp_Udm" required>
										<?php
										$sth = $conn_mes->prepare(
											"SELECT * FROM unita_misura"
										);
						...
						<div class="row">
							...
							<div class="col-6">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" name="con_IdRisorsa" id="con_IdRisorsa" data-live-search="true" required>
									</select>
								</div>
							</div>
							<div class="col-6">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" id="con_IdTipoConsumo" name="con_IdTipoConsumo" data-live-search="true" required>
							...
							<div class="col-6">
								<div class="form-group">
									...
									<select class="form-control form-control-sm dati-popup-modifica selectpicker" name="con_Rilevato" id="con_Rilevato" required>
							...
							<div class="col-6">
								<div class="form-group">
									<label for="con_ConsumoPezzoIpotetico">Consumo ipotetico per pezzo</label><span style='color:red'> *</span>
									<input type="number" class="form-control form-control-sm dati-popup-modifica" name="con_ConsumoPezzoIpotetico" id="con_ConsumoPezzoIpotetico" autocomplete="off" required>
								</div>
							</div>
					...
					<form class="forms-sample" id="form-dettagli-ordine">
						...
						<div class="row">
							...
							<div class="col-6">
								<div class="form-group">
									<label for="op_IdProduzione">Codice commessa</label>
									<input readonly type="text" class="form-control form-control-sm dati-popup-modifica obbligatorio" name="op_IdProduzione" id="op_IdProduzione">
								</div>
							</div>
							...
							<div class="col-6">
								<div class="form-group">
									...
									<input readonly type="text" class="form-control form-control-sm dati-popup-modifica" name="lp_Descrizione" id="lp_Descrizione">
								</div>
							</div>
							...
							<div class="col-8">
								<div class="form-group">
									<label for="prd_Descrizione">Prodotto</label>
									<input readonly type="text" class="form-control form-control-sm dati-popup-modifica" name="prd_Descrizione" id="prd_Descrizione">
								</div>
							</div>
							...
							<div class="col-4">
								<div class="form-group">
									...
									<input readonly type="text" class="form-control form-control-sm dati-popup-modifica" name="op_QtaDaProdurre" id="op_QtaDaProdurre">
								</div>
							</div>
							...
							<div class="col-6">
								<div class="form-group">
									...
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica" name="op_DataOraProduzione" id="op_DataOraProduzione">
								</div>
							</div>
							...
							<div class="col-6">
								<div class="form-group">
									...
									<input type="datetime-local" class="form-control form-control-sm dati-popup-modifica" name="op_DataOraFineTeorica" id="op_DataOraFineTeorica">
								</div>
							</div>



							<input type="hidden" id="rc_VelocitaRisorsa" name="rc_VelocitaRisorsa" value="">
							<input type="hidden" id="rc_IdRisorsa" name="rc_IdRisorsa" value="">
							<input type="hidden" id="op_QtaDaProdurreVal" name="op_QtaDaProdurreVal" value="">
	...
	<script src="../js/timelineordinirisorse.js"></script>
	<script src="../js/gestionecommesseestesa.js"></script>
